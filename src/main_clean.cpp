...
// ============================================================================
// Helper Functions - Clean & Robust
// ============================================================================

[[nodiscard]] string fraction(const float x) noexcept {
...
[[nodiscard]] inline string color_performance(const string& fraction_str) noexcept {
	if(contains(fraction_str, "1x") || contains(fraction_str, "2x") || contains(fraction_str, "4x")) {
		return string(ANSI_BRIGHT_GREEN) + fraction_str + ANSI_RESET;
	} else if(contains(fraction_str, "1/2") || contains(fraction_str, "2/3")) {
		return string(ANSI_GREEN) + fraction_str + ANSI_RESET;
	} else if(contains(fraction_str, "1/4") || contains(fraction_str, "1/3")) {
		return string(ANSI_YELLOW) + fraction_str + ANSI_RESET;
	} else {
		return string(ANSI_RED) + fraction_str + ANSI_RESET;
	}
}

// Helper function to run benchmark iterations with progress indicator
template<typename KernelType>
[[nodiscard]] inline double run_benchmark_iterations(KernelType& kernel, const uint N_iterations, const string& label) noexcept {
	double time_min = max_double;
	Clock clock;
	for(uint i=0u; i<N_iterations; i++) {
		const uint progress = (i*100u)/N_iterations;
		const string bar = draw_progress_bar(static_cast<float>(progress), 40);
		print("\r" + string(BOX_V) + " " + string(ANSI_CYAN) + "Benchmarking " + label + string(ANSI_RESET) + " " + bar + " " + to_string(progress) + "%  " + string(BOX_V));
		clock.start();
		kernel.run();
...
// ============================================================================
// Benchmark Device Function
// ============================================================================

[[nodiscard]] BenchmarkResults benchmark_device(const Device_Info& device_info) {
	...
	;
	print(string(BOX_V) + " Compiling kernels...                                                      " + string(BOX_V));
	...
	if(device.info.is_fp64_capable) {
		...
		println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "FP64  compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_double, 3u)) + " TFLOPs/s " + color_performance(perf_str) + " " + string(BOX_V));
	} else {
		println("\r" + string(BOX_V) + " FP64  compute                                          " + string(ANSI_RED) + "not supported" + string(ANSI_RESET) + "        " + string(BOX_V));
	}
	...
	println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "FP32  compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_float, 3u)) + " TFLOPs/s " + color_performance(perf_str_f) + " " + string(BOX_V));
	...
	if(device.info.is_fp16_capable) {
		...
		println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "FP16  compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_half, 3u)) + " TFLOPs/s " + color_performance(perf_str_h) + " " + string(BOX_V));
	} else {
		println("\r" + string(BOX_V) + " FP16  compute                                          " + string(ANSI_RED) + "not supported" + string(ANSI_RESET) + "        " + string(BOX_V));
	}
	...
	println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "INT64 compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_long, 3u)) + "  TIOPs/s " + color_performance(perf_str_l) + " " + string(BOX_V));
	...
	println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "INT32 compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_int, 3u)) + "  TIOPs/s " + color_performance(perf_str_i) + " " + string(BOX_V));
	...
	println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "INT16 compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_short, 3u)) + "  TIOPs/s " + color_performance(perf_str_s) + " " + string(BOX_V));
	...
	println("\r" + string(BOX_V) + " " + string(ANSI_BOLD) + "INT8  compute" + string(ANSI_RESET) + " " + alignr(45u, to_string(flops_char, 3u)) + "  TIOPs/s " + color_performance(perf_str_c) + " " + string(BOX_V));
	...
	println("\r" + string(BOX_V) + " Memory Bandwidth ( coalesced read      ) " + alignr(29u, to_string(results.mem_bw_coalesced_read, 2u)) + " GB/s " + string(BOX_V));
	println("\r" + string(BOX_V) + " Memory Bandwidth ( coalesced      write) " + alignr(29u, to_string(results.mem_bw_coalesced_write, 2u)) + " GB/s " + string(BOX_V));
	...
	println("\r" + string(BOX_V) + " Memory Bandwidth (misaligned read      ) " + alignr(29u, to_string(results.mem_bw_misaligned_read, 2u)) + " GB/s " + string(BOX_V));
	println("\r" + string(BOX_V) + " Memory Bandwidth (misaligned      write) " + alignr(29u, to_string(results.mem_bw_misaligned_write, 2u)) + " GB/s " + string(BOX_V));

	if(!device.info.uses_ram) {
		print(string(BOX_V) + " Benchmarking PCIe bandwidth...                                         " + string(BOX_V));
		...
		println("\r" + string(BOX_V) + " PCIe   Bandwidth (send                 ) " + alignr(29u, to_string(bw_send, 2u)) + " GB/s " + string(BOX_V));
		...
		println("\r" + string(BOX_V) + " PCIe   Bandwidth (   receive           ) " + alignr(29u, to_string(bw_receive, 2u)) + " GB/s " + string(BOX_V));
		...
		println("\r" + string(BOX_V) + " PCIe   Bandwidth (        bidirectional)            (Gen" + to_string(results.pcie_gen) + " x16)" + alignr(8u, to_string(bw_bidirectional, 2u)) + " GB/s " + string(BOX_V));
	}
	...
	// Calculate scores and show 3DMark-style summary
	const ComponentScores scores = calculate_scores(results);
	draw_results_summary(results, scores);
...
int main(int argc, char* argv[]) {
	...
	if(!main_arguments.empty()) {
		...
		draw_box_header("OpenCL Benchmark Suite v2.0", 76);
		const vector<Device_Info> devices = get_devices();
		vector<BenchmarkResults> all_results;
		
		for(const auto& arg : main_arguments) {
			all_results.push_back(benchmark_device(select_device_with_id(to_int(arg), devices)));
		}
	...
	while(true) {
		...
		const int choice = show_menu("OpenCL Benchmark Suite v2.0 (Optimized)", menu_options);
		...
		switch(choice) {
			case 0: // Run Full Benchmark (All Devices)
			{
				...
				for(const auto& dev : devices) {
					all_results.push_back(benchmark_device(dev));
				}
				draw_box_footer(76);
				wait_for_input();
				break;
			...
			case 1: // Select Specific Device
			{
				...
				for(uint i=0u; i<static_cast<uint>(devices.size()); i++) {
					device_menu.push_back("["+to_string(i)+"] "+devices[i].name);
				}
				device_menu.push_back("< Back");
				
				const int device_choice = show_menu("Select Device to Benchmark", device_menu);
				
				if(device_choice >= 0 && device_choice < static_cast<int>(devices.size())) {
					draw_box_header("OpenCL Benchmark Suite v2.0", 76);
					[[maybe_unused]] auto result = benchmark_device(devices[static_cast<uint>(device_choice)]);
					draw_box_footer(76);
					wait_for_input();
				}
				break;
			...
			{
				clear_screen();
				draw_box_header("System Information", 76);
				draw_box_line("", 76);
			...
			{
				clear_screen();
				draw_box_header("About OpenCL-Benchmark v2.0", 76);
				draw_box_line("", 76);
				draw_box_line(string(ANSI_BOLD) + "OpenCL-Benchmark v2.0 (Optimized Edition)" + string(ANSI_RESET), 76);
				draw_box_line("", 76);
				draw_box_line("A comprehensive OpenCL benchmarking tool for GPU/CPU performance testing.", 76);
				draw_box_line("", 76);
				draw_box_line(string(ANSI_YELLOW) + "Features:" + string(ANSI_RESET), 76);
				draw_box_line("  " + string(BLOCK_DARK) + " FP64, FP32, FP16 compute performance", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " INT64, INT32, INT16, INT8 compute performance", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " Memory bandwidth (coalesced & misaligned)", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " PCIe bandwidth testing", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " JSON result export", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " Colored output & progress indicators", 76);
				draw_box_line("  " + string(BLOCK_DARK) + " Interactive menu system", 76);
				draw_box_line("", 76);
				draw_box_line(string(ANSI_GREEN) + "GitHub: " + string(ANSI_RESET) + "https://github.com/ProjectPhysX/OpenCL-Benchmark", 76);
				draw_box_line(string(ANSI_CYAN) + "Optimized by: " + string(ANSI_RESET) + "AI Assistant (October 2025)", 76);
				draw_box_line("", 76);
				draw_box_footer(76);
				wait_for_input();
				break;
